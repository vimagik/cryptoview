<template>
  <div class="container mx-auto flex flex-col items-center bg-gray-100 p-4">
    ...
    <div class="container">
      ...
      <hr v-if="massOfTikers.length !== 0" class="w-full border-t border-gray-600 my-4"/>
      <dl class="mt-5 grid grid-cols-1 gap-5 sm:grid-cols-3">
        <div
            class="bg-white overflow-hidden shadow rounded-lg border-purple-800 border-solid cursor-pointer"
            v-for="tick of massOfTikers"
            :key="tick.id"
            @click="takeNewElement(tick)"
            :class="{
              'border-4': tick===currentElement
            }"
        >
          ...
          <button
              ...
              @click.stop="deleteTicker(tick.id)"
          >
            <svg
                class="h-5 w-5"
                xmlns="http://www.w3.org/2000/svg"
      ...
      <hr v-if="massOfTikers.length !== 0" class="w-full border-t border-gray-600 my-4"/>
      <section class="relative" v-if="currentElement">
        <h3 class="text-lg leading-6 font-medium text-gray-900 my-8">
          {{ currentElement.name }} - USD
        </h3>
        <div class="flex items-end border-gray-600 border-b border-l h-64">
          <div
              class="bg-purple-800 border w-10"
              v-for="(bar, idx) of normolizeGraph()"
              :key="idx"
              :style="{ height: `${bar}%`}"
          ></div>
        </div>
...
export default {
  ...
  methods: {
    ...
    add(tiketName = '') {
      ...
      setInterval(async () => {
        const f = await fetch(`https://min-api.cryptocompare.com/data/price?fsym=${newTicker.name}&tsyms=USD&api_key=89dea0a16cdcafec09b2b4e1f829e7cef15085dfa81f0b6c04cf5baf44e3460f`);
        const response = await f.json();
        let currentTicker = this.massOfTikers.find(element => element.name === newTicker.name)
        currentTicker.price = response.USD > 1 ? response.USD.toFixed(2) : response.USD.toPrecision(2);
        if (newTicker.name === this.currentElement?.name) {
    ...
    deleteTicker(key) {
      this.massOfTikers = this.massOfTikers.filter((tiker) => (tiker.id !== key));
      if (this.currentElement.id === key) {
        this.clearCurrentElement()
      }
    },

    takeNewElement(element) {
    ...
    async getAvaliableCoinList() {
      ...
      for (let key in response.Data) {
        let newTicket = {
          ...
          partner_symbol: response.Data[key]['partner_symbol']
        }
